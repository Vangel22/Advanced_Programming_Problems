...
class Timestamp<T> implements Comparable<Timestamp<T>>{
    ...
    public boolean equals(Object o) {
        ...
        @SuppressWarnings("unchecked") //ova e za da ne mi bara refaktoriranje
        Timestamp<T> timestamp = (Timestamp<T>) o;
        if(date == null){
            if(timestamp.date != null){
                return false;
            }
        }else if(!date.equals(timestamp.date)){
            return false;
        }
        if(element == null){
            if(timestamp.element != null){
                return false;
            }
        }else if(!element.equals(timestamp.element)){
            return false;
        }
        return true;
    }
...
class Scheduler<T>{
    ...
    public Scheduler(ArrayList<Timestamp<T>> list) {
        this.list = list;
    }
    ...
    //ovde proverka
    public boolean remove(Timestamp<T> timestamp){ //vaka ne go brise
        if(list.isEmpty()){
            return false;
        }
    ...
    /*//ovde proverka
    public boolean elementExists(Timestamp<T> timestamp){
        boolean check = false;
        for(Timestamp<T> elements : list){
            if(elements.equals(timestamp))
                return true;
        }
        return check;
    }*/

    //go vrakja sledniot Timestamp objekt cie vreme e najblisko do tekovnoto i ne e pominato
    public Timestamp<T> next(){
        LocalDateTime now = LocalDateTime.now();
        Collections.sort(list);
        for(int i=0; i<list.size(); i++){
            if(list.get(i).getTime().compareTo(now) >= 0){
                return list.get(i); //mi go vrakja sledniot objekt najblisku do tekovnoto vreme
            }
        }
        return null;
    }

    //vrakja objekt so vreme najblisku do tekovnoto i e veke pominato
    public Timestamp<T> last(){
        ...
        return null;
    } //.get() metodot zema element od listata na konkreten indeks

    //ovoj metod treba da vrati lista od intervalot pomegju begin i end bez begin i end
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        ArrayList<Timestamp<T>> lista = new ArrayList<>();
        for(int i=0; i<list.size(); i++){
